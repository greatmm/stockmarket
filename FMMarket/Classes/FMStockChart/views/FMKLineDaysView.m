...
@interface FMKLineDaysView()<UIScrollViewDelegate>{

}

@end

@implementation FMKLineDaysView
...
-(void)startGetHttpStockDays{
    ...
    __FMWeakSelf;
    [HttpManager getHttpStockDaysWithCode:self.model.stockCode fuquanType:[self fuquanType:self.model.fuquanType] StartBlock:^{
...
//            model = nil;
        ...
        NSString *localData = [MKUserDefault getSeting:[NSString stringWithFormat:@"%@_%@",fmFMKLineDaysDataLocalKey,self.model.stockCode]];
        if (localData) {
            ...
            if (![datetime isEqualToString:last.datetime]) {
                ...
                m.volumn = [NSString stringWithFormat:@"%.2f",[newData.volumn floatValue]/100];
